...
class Config:
    ...
    max_open_positions: int = 2          # ✅ رُفع إلى 2
    ...
    scanner_min_atr_pct: float = 0.25     # ✅ خُفّض
    ...
    db_path: str = "trades_v2.db"       # ✅ DB جديدة
...
# ✅ دالة تشخيص الرفض — ستسجل السبب بدقة
def explain_rejection(m: MSSIOutput) -> List[str]:
    reasons = []
    if m.direction_bias < CFG.min_long_score and m.direction_bias > 50:
        reasons.append(f"LONG bias too low: {m.direction_bias:.1f} < {CFG.min_long_score}")
    if m.direction_bias < 50 and m.direction_bias > (100 - CFG.min_short_score):
        reasons.append(f"SHORT bias too weak: {m.direction_bias:.1f} > {100-CFG.min_short_score}")
    if m.entry_quality < CFG.min_entry_quality:
        reasons.append(f"Entry quality low: {m.entry_quality:.1f} < {CFG.min_entry_quality}")
    if m.risk_score > CFG.max_risk_for_entry:
        reasons.append(f"Risk high: {m.risk_score:.1f} > {CFG.max_risk_for_entry}")
    if m.continuation_probability <= m.reversal_probability:
        reasons.append(f"Reversal dominates: CONT={m.continuation_probability:.1f}, REV={m.reversal_probability:.1f}")
    if not reasons:
        reasons.append("Unknown reason — check feature values")
    return reasons
...
class MSSIEngine:
    ...
    def score(self, f: RawFeatures, regime: MarketRegime, direction: TrendDirection) -> MSSIOutput:
        ...
        # ✅ حساب الثقة أولاً — قبل اتخاذ القرار
        m.confidence = (
...
class MarketScanner:
    ...
    def _deep(self,sk,sym):
        ...
        # ✅ تسجيل تشخيص الرفض بوضوح
        if mssi.decision == "WAIT":
            reasons = explain_rejection(mssi)
            logger.warning(f"WAIT {sym} | CONF={mssi.confidence:.1f} | " + " | ".join(reasons))
        ...
        if mssi.decision == "WAIT":
            ...
            final.final_score = mssi.confidence  # الآن غير صفر دائمًا
            ...
            final.final_score = mssi.confidence  # لا نخصم
        ...
        if final.decision == Decision.WAIT:
            return  # لن يدخل — لكن الآن نعرف لماذا
