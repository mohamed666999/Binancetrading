...
class Config:
    ...
    min_long_score: float = 54.0       # ✅ خُفضت من 58
    min_short_score: float = 46.0      # ✅ رُفعت من 42
    max_risk_for_entry: float = 85.0   # ✅ رُفعت من 82
    min_entry_quality: float = 25.0    # ✅ خُفضت من 28
    min_confidence: float = 28.0       # ✅ خُفضت من 32
    ...
    db_path: str = "trades_v3.db"      # ✅ اسم جديد = ذاكرة نظيفة
...
# ✅ التعديل 2: execute_trade المحصنة (تمنع تكرار فتح نفس الصفقة)
def execute_trade(sym, final):
    with execution_lock:
        try:
            # 🛡️ الجدار الأول: فحص قاعدة البيانات المحلية أولاً (يمنع فتح صفقة فوق صفقة)
            ...
            if db.count_today() >= CFG.max_daily_trades: logger.info("🚫 تم الوصول للحد اليومي للصفقات"); return
            if db.open_count() >= CFG.max_open_positions: logger.info("🚫 تم الوصول للحد الأقصى للصفقات المفتوحة"); return
            ...
            logger.info(f"🚀 بدء التنفيذ {sym} | {pname} | السعر: {price}")
            st["t"] = time.time() 
            ...
            time.sleep(2) # 🛡️ السماح لبينانس بتحديث بياناتها
            
            p = get_pos(sym)
            if p == "ERROR" or p is None: 
                logger.critical(f"❌ لم يتم العثور على المركز بعد فتحه لـ {sym}"); return
...
class PositionMonitor:
    ...
    # ✅ التعديل 3: _check مع فترة السماح 60 ثانية (تمنع الفزع والإغلاق الوهمي)
    def _check(self, trade):
        sym = trade["symbol"]
        
        # 🛡️ فترة سماح: البوت لن يتدخل لتقييم الصفقة إذا لم يمر على فتحها 60 ثانية
        try:
    ...
    def _rexit(self,sym,trade):
        ...
        return ep,rpnl,comm
    # ✅ التعديل 4: _cancel مع تتبع الأخطاء والتنظيف الجراحي
...
def main():
    ...
    logger.info(f"   DirBias Long>={CFG.min_long_score} Short<={100-CFG.min_short_score}")
    logger.info(f"   DB: {CFG.db_path} (Fresh)")
    logger.info("="*60)
    ip_monitor.start()
